...
import DetailCashInvoiceHeader from "../../invoice/DetailCashInvoiceHeader";
import DetailCashInvoice from "../../invoice/DetailCashInvoice";
...
function EnhancedTableHead(props) {
    ...
    const headCells = [
        {id: 'invoiceType', numeric: false, disablePadding: false, label: t("INVOICETYPE1")},
        {id: 'invoicePrice', numeric: false, disablePadding: false, label: t('INVOICEAMOUNT')},
        {id: 'cashCurrency', numeric: false, disablePadding: false, label: t('CASHCURRENCY')},
        {id: 'cashDate', numeric: false, disablePadding: false, label: t('DATE')},
...
function ExpenseDelete(props: any) {
    ...
    const steps = [t('EXPENSEDETAILS'), t('CONFIRMATION')];
    ...
    useEffect(()=>{
        console.log("cash", cashInvoice)
    }, [cashInvoice])

    useEffect(() => {
        // @ts-ignore
        if (window.atob(taskid)) {
            api.getExpenseByTaskId(window.atob(taskid)).then(res => {
                console.log(res,"delete")
                setCashInvoice(res)
                api.getExpensesTypeById(res?.cashInvoiceTypeId).then(res => {
                    setExpensesType(res)
                })
            })
                        ...
                        <div className="flex flex-1 flex-col items-center sm:items-start">
                            ...
                            <div className="flex flex-col min-w-0 items-center sm:items-start">
                                <Typography className="text-16 sm:text-20 truncate">
                                    {t('EXPENSEDELETEREQUEST')}{` - ${window.atob(taskid)}`}
                                </Typography>
                        ...
                        <PopupState variant="popover" popupId="demo-popup-menu">
                            {(popupState) => (
                                <React.Fragment>
                                    ...
                                    <Button variant="contained" color="secondary" {...bindTrigger(popupState)} disabled={cashInvoice?.task?.taskConfirmations?.[cashInvoice?.task?.taskConfirmations?.length - 1]?.taskConfirm == "DONE"}>
                                    ...
                                    <TableBody>
                                        ...
                                        >
                                            ...
                                            <TableCell align="left">
                                                <a href={getDocumentUrlByFileName(cashInvoice?.fileName)}>{cashInvoice?.fileName}</a>
                                            </TableCell>
                                        </TableRow>
                                    </TableBody>
                                </Table>
                        ...
                        {activeStep === 1 && (
                            <div className="table-responsive">
                                ...
                                <table className="simple">
                                    ...
                                    <tbody>
                                    {/*@ts-ignore*/}
                                    {cashInvoice?.task?.taskConfirmations?.map(confirm => (
                                        <tr key={confirm.id}>
                                            <th>
