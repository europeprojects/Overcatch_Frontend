...
import {
    ...
    MenuItem,
    Grid,
    Select, Input, ListItemText, Button, DialogTitle, DialogContent, TextField, Box, Card, FilledInput, FormLabel
...
function EnhancedTableHead(props) {
    ...
    const {allType, setAllType} = props
    ...
    const {personel, setPersonel, selectedPersonel, setSelectedPersonel, personelId, setPersonelId}=props;

    const headCells = [
        { id: 'all1', numeric: false, disablePadding: false, label: t("ALL"),sortable:false},
    ...
    const { confirmTypeClick, setConfirmTypeClick} = props;
    const {moduleList} = props
    const [personName, setPersonName] = React.useState<string[]>([]);

    function handleChangeCheckbox(value: string, checked: boolean, type: string) {
        if (type == 'all1'){
            if(checked && !allType.includes(value))
                setAllType(oldArray => [...oldArray,value])
            else if(!checked && allType.includes(value))
                setAllType(allType.filter(item => item != value))
            return
        }
            ...
            {headCells.map(headCell => headCell.id=="moduleType" || headCell.id=="confirmType"
                ...
                :	(
                    headCell.id == "clientType" || headCell.id == "agreementType" ?
                        <TableCell
                            key={headCell.id}
                            style={{ padding: "2px" ,textAlign: 'start',minWidth:"76px" }}
                        ...
                        headCell.id == "confirmDate" || headCell.id == "processDate" ?
                            <TableCell
                                ...
                                align={"center"}
                            >
                                {headCell.id == "confirmDate" ? t("CONFIRMDATE") : t("ORDERDATE")}
                                <TextField
                                    style={{ marginTop:'-3px'}}
                                    type="date"
                                    size={'small'}
                                ...
                                >
                                    ...
                                    >
                                            <MenuItem key={"ADMIN"} value={"ADMIN"}>
                                                <Checkbox onChange={(e,checked) => {handleChangeCheckbox("ADMIN", checked, headCell.id)}}
                                                          checked={allType.indexOf("ADMIN") > -1} />
                                                <ListItemText primary={"ADMIN"} />
                                            </MenuItem>
                                            <MenuItem key={"HELP_MODULE"} value={"HELP_MODULE"}>
                                                <Checkbox onChange={(e,checked) => {handleChangeCheckbox("HELP_MODULE", checked, headCell.id)}}
                                                          checked={allType.indexOf("HELP_MODULE") > -1} />
...
export default function TaskList(props: any) {
    ...
    function handledetails(task: any) {
        if(task?.moduleTypeEnum == "HELP_MODULE"){
            // @ts-ignore
            history.push({
                pathname: '/SupportTask',
                displayRouteData: {
                ...
                <TableContainer className={classes.container}>
                    ...
                    >
                        <EnhancedTableHead
                            ...
                            personel={props.personel} setPersonel={props.setPersonel}
                            personelId={props.personelId} setPersonelId={props.setPersonelId}
                            selectedPersonel={props.selectedPersonel} setSelectedPersonel={props.setSelectedPersonel}
                        />
                                    ...
                                    return (
                                        ...
                                        >
                                            <TableCell onClick={event => handledetails(task)} style={{ fontSize:'12px' , fontWeight: 'bold' , marginLeft:'5px'}} scope="row" >
                                                {task?.moduleTypeEnum=="HELP_MODULE"?"SUPPORT":"ADMIN"}
                                            </TableCell>
                                            <TableCell onClick={event => handledetails(task)} style={{ fontSize:'12px' , fontWeight: 'bold' , marginLeft:'5px'}} scope="row" >
                                                {task.moduleTypeEnum}
                                            </TableCell>
