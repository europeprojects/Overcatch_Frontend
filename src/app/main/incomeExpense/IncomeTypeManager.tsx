...
import {createStyles, lighten, makeStyles, Theme, ThemeProvider, useTheme} from '@material-ui/core/styles';
...
import {
    ...
    Typography, TableContainer, TextField} from '@material-ui/core';
import clsx from 'clsx';
import DeleteIcon from '@material-ui/icons/Delete';
import FilterListIcon from '@material-ui/icons/FilterList';
import {func, number, object, oneOf, string} from 'prop-types';
import {IncomesType} from "../../types/UserModel";
...
import { incomeScheme} from "../validations/ValidationSchemes";
...
const headCells = [
    {id: 'type', numeric: false, disablePadding: true, label: 'Income Type'},
...
function IncomeTypeManager(props: any) {
    ...
    const [buyer, setBuyer] = useState<IncomesType>();
    const [incomesList, setIncomesList] = React.useState([]);
    const [open, setOpen] = React.useState(false);
    const [openDetail, setOpenDetail] = React.useState(false);
    const [openDelete, setOpenDelete] = React.useState(false);
    const [selectedType, setSelectedType] = useState();
    const [income, setIncome] = useState<IncomesType>({} as IncomesType);
    ...
    const handleSave = (values) => {
        ...
        let incomesType: IncomesType;
        incomesType = {
            id: null,
            incomesType: formik.values.incomesType
        }

        api.saveIncomeType(incomesType).then(res => {
            getIncomesType()
        })
    ...
    const handleSaveDetail = (values) => {
        //@ts-ignore
        api.controlIncomeForDelete(selectedType?.id).then(
            (res) => {
                console.log(res)
                if (res < 1) {
                    // @ts-ignore
                    api.updateIncomesType(income).then(res => {
                        handleClose();
                        getIncomesType()
                    })
                } else {
    ...
    {
        let filteredData = incomesList.filter(incomes =>incomes.incomesType.toLowerCase().includes(searched.toLowerCase())).map((incomes)=>{
            return incomes;
        })
    ...
    const handleDelete = (event) => {
        ...
        api.controlIncomeForDelete(selectedType?.id).then(
            (res)=>{
                if(res<1){
                    // @ts-ignore
                    api.deleteIncomeTypeById(selectedType?.id).then(res=>{setIncomesList(incomesList?.filter(i => i.id !== selectedType?.id))});
                    setOpenDelete(false)
                    getIncomesType()
                }else{
                    props.enqueueSnackbar(<h4>{t("CANTDELETE")}</h4>, {
                                                    ...
                                                    >
                                                        <Grid item xs>
                                                            <TextField
                                                                ...
                                                                label={t("")}
                                                                variant="outlined"
                                                                value={formik.values.incomesType}
                                                                onChange={formik.handleChange}
                                                                error={formik.touched.incomesType && Boolean(formik.errors.incomesType)}
                                                                helperText={formik.touched.incomesType && formik.errors.incomesType}
                                                                fullWidth={true}
                                                ...
                                                >
                                                    <Grid item xs>
                                                        <TextField
                                                            ...
                                                            variant="outlined"
                                                            value={income.incomesType}
                                                            onChange={(e)=>{setIncome({...income,incomesType:e.target.value})}}
                                                            fullWidth={true}
                                                            className={"object-center"}
                                                        />
                                                    </Grid>
...
export default withSnackbar(IncomeTypeManager);